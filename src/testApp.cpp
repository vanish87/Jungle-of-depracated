...
//    }
    ...
    model.loadModel("test_3.dae",true);
...
void testApp::draw(){
    ...
    for (int deviceID = 0; deviceID < numDevices; deviceID++){
       ...
       // openNIDevices[deviceID].drawDebug(); // debug draw does the equicalent of the commented methods below
        openNIDevices[deviceID].drawDepth(0, 0, 320, 240);
        openNIDevices[deviceID].drawImage(960, 0, 320, 240);
        openNIDevices[deviceID].drawSkeletons(960, 0, 320, 240);
